        ...
        if st.button("Acessar"):
            if u == "admin" and p == "2153App":
                ...
                try:
                    ...
                    if res.data:
                        user = res.data[0] # Pega o primeiro usuário da lista
                        if user['status'] == 'ativo':
                            # CORREÇÃO DE DATA: Compara datetimes com fuso horário (timezone aware)
                            data_ap = datetime.fromisoformat(user['data_aprovacao'].replace('Z', '+00:00'))
...
with tab1:
    ...
    with c4: st.write("**Altura [A]**"); st.write(f"### {alt} mm")
    with c5: st.write("**Largura [L]**"); st.write(f"### {larg} mm")
    with c6: st.write("**Profundidade [P]**"); st.write(f"### {prof} mm")
...
with tab2:
    ...
    if st.button("Calcular Resultados"):
        ...
        st.divider(); st.write("### Resultados:"); st.write(f"**Iarc Final:** {ia_f:.4f} kA"); st.write(f"**Energia:** {e_cal:.4f} cal/cm²"); st.warning(f"🛡️ Vestimenta: {cat}")
