                ...
                try:
                    ...
                    if res.data:
                        ...
                        if user_data['status'] == 'ativo':
                            # Verificação de 1 ano com fuso horário corrigido
                            ...
                            st.warning(f"Seu acesso está: {user_data['status'].upper()}. Aguarde aprovação.")
                    else:
...
with tab1:
    ...
    with c4: st.write(f"**Altura [A]:** {alt} mm")
    with c5: st.write(f"**Largura [L]:** {larg} mm")
    with c6: st.write(f"**Profundidade [P]:** {prof} mm")
...
with tab2:
    ...
    if st.button("Calcular Resultados"):
        ...
        ia_sts = [calc_ia_step(i_bf, gap_g, k_ia[v]) for v in]; en_sts = [calc_en_step(ia, i_bf, gap_g, dist_d, tempo_t, k_en[v], cf) for ia, v in zip(ia_sts,)]; dl_sts = [calc_dla_step(ia, i_bf, gap_g, tempo_t, k_en[v], cf) for ia, v in zip(ia_sts,)]
